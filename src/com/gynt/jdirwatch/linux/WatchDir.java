package com.gynt.jdirwatch.linux;
...
public class WatchDir {
		...
		for (;;) {
			...
			if (key == null) {
				if (events.size() > 0 && System.currentTimeMillis() - lastEvent > cutoff) {
					...
					events.clear();
					lastEvent = 0;
					System.out.format("EventSet: total: %s, path: %s\n", set.getEvents().length,
							set.getEvents()[0].getFullPath());
				}
				try {
					Thread.sleep(100);
			...
			// TODO: Consider a single poll to be a chunk of events... Not sure how this
			// works with slow computers.
	...
	public static void main(String[] args) throws IOException {
		// parse arguments
		args = new String[] { "-r", "/home/frank/Projects/JDirWatch/test" };
		
		//Does not make folder modify events!!!
		
		//args = new String[] {"/home/frank/Projects/JDirWatch/test/test.txt"};
